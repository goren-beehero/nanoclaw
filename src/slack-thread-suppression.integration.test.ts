...
const { TEST_DIR, resolveThreadRootUserId } = vi.hoisted(() => ({
...
function event(id: string, threadId: string, mention = false, channel = 'C-TEST'): InboundEvent {
  return {
    channelType: 'slack',
    platformId: channel,
    threadId,
    message: {
      id,
      kind: 'chat-sdk',
      content: JSON.stringify({ senderId: 'U-SENDER', text: mention ? '<@U-BOBI> help' : 'ambient question' }),
      timestamp: now(),
      isMention: mention,
      isGroup: true,
    },
...
describe('Slack suppression router integration', () => {
  it('suppresses before session creation, typing, and wake', async () => {
    ...
    await routeInbound(event('100.1', 'slack:C-TEST:100.1'));
  ...
  it('keeps replies by other users suppressed when the root is blacklisted', async () => {
    resolveThreadRootUserId.mockResolvedValue('U-BLACKLISTED');
    const { routeInbound } = await import('./router.js');
    await routeInbound(event('200.1', 'slack:C-TEST:200.1'));
    await routeInbound(event('200.2', 'slack:C-TEST:200.1'));
  ...
  it('allows a resolved mention and ordinary follow-up in the same thread exactly once', async () => {
    ...
    await routeInbound(event('300.1', 'slack:C-TEST:300.1', true));
