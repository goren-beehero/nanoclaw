 ...
 * @whiskeysockets/baileys 7.0.0-rc14 (pinned; includes the message-spoofing fix
 * from GHSA-qvv5-jq5g-4cgg).
...
/**
 * Preserve an unknown sender's approval flow without fetching their media.
 * Once approved, the agent sees this note and asks for a resend; no attacker
 * bytes were downloaded or persisted while authorization was pending.
 */
export function appendMediaWithheldNote(content: string, mediaTypes: string[]): string {
  if (mediaTypes.length === 0) return content;
  const note = `[${mediaTypes.join(', ')} withheld until sender authorization; please resend after approval]`;
  return content ? `${content}\n${note}` : note;
}

/**
 * Single fail-closed seam around every inbound media fetch. Exported so the
 * regression test can prove a denied/legacy host never invokes the downloader.
 */
export async function runAuthorizedMediaDownload<T>(
  setup: ChannelSetup,
  platformId: string,
  message: InboundMessage,
  download: () => Promise<T>,
): Promise<{ authorized: false } | { authorized: true; value: T }> {
  const authorized = (await setup.authorizeInboundMedia?.(platformId, null, message)) === true;
  if (!authorized) return { authorized: false };
  return { authorized: true, value: await download() };
}
...
registerChannelAdapter('whatsapp', {
  factory: () => {
    ...
    /** Download admitted media and hand bytes to the host's session inbox staging. */
    async function downloadInboundMedia(
      msg: WAMessage,
      // Baileys' normalized message is a heterogeneous proto union keyed by
      // message kind; keep that library boundary local to this helper.
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      normalized: any,
    ): Promise<{
      ...
      for (const { key, type, ext } of mediaTypes) {
        ...
        try {
          ...
          // WhatsApp's E2E channel — Meta can't sanitize it server-side. Keep
          // the basename guard even though the host re-validates before staging.
          ...
          if (rawFilename && filename !== rawFilename) {
            log.warn('Refused unsafe attachment filename — would escape session inbox', {
              rawFilename,
    ...
    async function connectSocket(): Promise<void> {
      ...
      if (phoneNumber && !state.creds.me) {
        setTimeout(() => {
          void sock
            ...
            .catch((err) => log.error('Failed to request pairing code', { err }));
        }, 3000);
      }
      ...
      sock.ev.on('connection.update', (update) => {
        ...
        if (qr && !phoneNumber) {
          ...
          void import('qrcode')
            .then((QRCode) => QRCode.toString(qr, { type: 'terminal' }))
            .then((qrText) => log.info('WhatsApp QR code — scan with WhatsApp > Linked Devices:\n' + qrText))
            .catch(() => log.info('WhatsApp QR code (raw)', { qr }));
        }
      ...
      sock.ev.on('creds.update', () => {
        void saveCreds().catch((err) => log.error('Failed to persist WhatsApp credentials', { err }));
      });
      ...
      sock.ev.on('messages.upsert', ({ messages }) => {
        void (async () => {
          for (const msg of messages) {
            try {
              ...
              let attachments: Array<{ type: string; name: string; data: string; size: number }> = [];
              let failures: string[] = [];
              if (inboundMediaTypes.length > 0) {
                const admitted = await runAuthorizedMediaDownload(setupConfig, chatJid, inboundBase, () =>
                  downloadInboundMedia(msg, normalized),
                );
                if (admitted.authorized) {
                  attachments = admitted.value.attachments;
                  failures = admitted.value.failures;
                } else {
                  content = appendMediaWithheldNote(content, inboundMediaTypes);
                  log.info('Inbound media withheld before sender authorization', {
                    chatJid,
                    sender,
                    mediaTypes: inboundMediaTypes,
                  });
                }
              }
              ...
              const inbound: InboundMessage = {
                ...inboundBase,
                content: {
                  ...(inboundBase.content as Record<string, unknown>),
                  text: content,
        ...
        })().catch((err) => log.error('WhatsApp inbound batch failed', { err }));
      });
    }
